angular.module('starter').controller('loginCtrl',
  ['$scope','$efficiencyApi', '$state', '$sharedData', function($scope, $efficiencyApi, $state, $sharedData){
    $scope.login = function(){
      $efficiencyApi.login($scope.user, $scope.password).then(function(user){
      if(user.data[0]){
        $sharedData.setUser(user.data);
        $state.go('app');
      }else{
...
angular.module('starter').controller('typeSelectionCtrl',
  ['$scope', '$state', '$sharedData', function($scope, $state, $sharedData){
